...
if (sheet.student.toString() !== req.user.id) {
  return res.status(403).json({ message: 'You can only request a recheck for your own answer sheet.' });
}
...
if (req.user.role === 'student' && recheck.student._id.toString() !== req.user.id) {
  return res.status(403).json({ message: 'Not authorized to view this recheck request.' });
}
...
if (['professor', 'ta'].includes(req.user.role)) {
  ...
  if (
    (req.user.role === 'professor' && courseData.professor.toString() !== userId) ||
    (req.user.role === 'ta' && !courseData.TAs.map(taId => taId.toString()).includes(userId))
  ) {
    return res.status(403).json({ message: 'Not authorized to view this recheck request.' });
  }
}
...
if (
  ...
  return res.status(403).json({ message: 'Not authorized to update this recheck request.' });
}
...
module.exports = { createRecheckRequest, getRecheckRequests,getRecheckRequestById, updateRecheckRequest };